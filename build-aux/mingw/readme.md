Building Solbit on Native Windows (Semi-Automated Install & Configuration)
===========================================================================

This document describes how to use the included scripts to perform a semi-
automated installation & configuration of the full Solbit development
...
2. Clone the Solbit Unlimited repository.
...
2. Execute `/build-aux/mingw/rebuild-solbit.bat` to build solbit from source.
...
Once git is installed, you should clone the Solbit Unlimited repository.  This
...
3. Clone the Solbit Unlimited repository with the following command:
...
Now that the Solbit Unlimited source code is downloaded, you should open the
...
5. Set the `DEPS_ROOT` path.  This is the path where all dependencies needed to
   build solbit will be downloaded and built.  Within this path, an x86 and/or
   ...
   scripts to work.**
6. NOTE: The `SOLBIT_GIT_ROOT` variable should only be modified if you have the
...
7. `BUILD_32_BIT` is commented out by default.  If you wish to build 32-bit
   versions of the Solbit executables, uncomment this line.
...
10. `ENABLE_TESTS` is disabled by default.  As the tests do not currently run
   ...
   you run a full rebuild of all dependencies, as well as the Solbit client, as
...
11. `CLEAN_BUILD` is disabled by default.  This will force a `make clean` prior
   to build of the Solbit client.
   NOTE: This only affects the build of the Solbit client, it does not affect
...
13. `SKIP_AUTOGEN` is disabled by default.  This is a convenience feature that
   should only be used when rebuilding Solbit, not while doing the initial
...
Solbit binaries as either 32-bit or 64-bit.  The configuration scripts allow
...
The initial configuration process is also set up to build the Solbit client
