...
if "%BUILD_ARCH%" NEQ "T" (
	...
	echo of the Solbit client in SET_ENV_VARS.bat.
...
if not exist "%MINGW_ROOT%" (
	...
	echo Aborting build of Solbit client...
	pause
	exit /b -1
)
...
if not exist "%MSYS_SH%" (
	echo MSYS does not appear to have installed correctly.  Build of Solbit client
...
if "%BUILD_32_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%SOLBIT_GIT_ROOT%\build-output\x86"
...
if "%BUILD_64_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%SOLBIT_GIT_ROOT%\build-output\x64"
...
%MSYS_SH% "%INST_DIR%\make-solbit.sh"
REM Check to see if make-solbit.sh failed (possibly due to missing dependencies)
if %errorlevel% neq 0 (
...
echo Saving solbit executables to %BUILD_OUTPUT%
REM Make sure output directory exists
mkdir "%BUILD_OUTPUT%\"

REM cd to src to copy solbit-tx.exe, solbit-cli.exe, and solbitd.exe
cd "%SOLBIT_GIT_ROOT%\src"
copy solbit-tx.exe "%BUILD_OUTPUT%\solbit-tx.exe"
copy solbit-cli.exe "%BUILD_OUTPUT%\solbit-cli.exe"
copy solbitd.exe "%BUILD_OUTPUT%\solbitd.exe"
copy solbit-miner.exe "%BUILD_OUTPUT%\solbit-miner.exe"

REM cd to src\qt to copy solbit-qt.exe
cd qt
copy solbit-qt.exe "%BUILD_OUTPUT%\solbit-qt.exe"
