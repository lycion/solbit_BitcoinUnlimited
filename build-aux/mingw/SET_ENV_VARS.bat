...
REM          of your solbit source under ".\build-aux\mingw\".  If this is true, you do not need
REM          to modify the SOLBIT_GIT_ROOT path.  Otherwise update this with the absolute path to
...
REM The latest versions of the solbit client uses Python3, while previous versions use Python2
...
REM Set the dependency path.  This is where all of the Solbit dependencies will be downloaded and built
...
REM Set the path to your Solbit git checkout.
REM NOTE: If you are running these scripts from the solbit checkout location, you do not need to modify this
set "SOLBIT_GIT_ROOT=%CD%\..\..\"


REM ##################################################################################################
REM These parameters configure how the solbit client and dependencies are built.
REM
...
REM    6. Strip enabled
REM    7. Skip Solbit autogen.sh step disabled (useful for rebuilding local changes only)
REM    8. Skip Solbit configure step disbled (useful for rebuilding local changes only)
REM
...
REM Following will strip debug symbols from the generated solbit executables, greatly reducing file size.
REM This will, however, make it more difficult to debug any issues that may occur while testing.
REM If you want to keep debug symbols in the generated solbit executables, uncomment the line below.
SET STRIP=YES

REM If you want to skip running "./autogen.sh" when building solbit uncomment the line below.
...
REM NOTE: This only affects the build of the solbit executables, not any of the dependencies.
...
REM If you want to skip running "./configure" when building solbit uncomment the line below.
REM This is useful to turn off if you have already run this once and not changed the toolchain
