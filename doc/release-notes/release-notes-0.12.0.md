Solbit Core version 0.12.0 is now available from:
...
backwards-compatible with pre-0.10 versions of Solbit Core or other software:
...
ECDSA signatures inside Solbit transactions now use validation using
...
Previous versions of Solbit Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Solbit Core was to raise the default minimum
relay fee.

Solbit Core 0.12 will have a strict maximum size on the mempool. The
...
Solbit Core 0.12 also introduces new default policy limits on the length and
...
Solbit Core 0.12 nodes. Solbit Core will only allow replacement of
...
Note that the wallet in Solbit Core 0.12 does not yet have support for
...
Solbit Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Solbit Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Solbit Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Solbit Core 0.13, and it is at this time undecided whether the
...
Solbit Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Solbit Core automatically creates a hidden service to listen on, without
manual configuration. Solbit Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Solbit Core is listening, and
...
Solbitd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Solbit
Core to use floating fees. By default, Solbit Core will use floating
fees.
...
(default: `0.0002` SBT/kB).

At all times, Solbit Core will cap fees at `-maxtxfee=<x>` (default:
0.10) SBT.
Furthermore, Solbit Core will never create transactions paying less than
...
- #6214 `87406aa` [OSX] revert renaming of Solbit-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #7209 `f3ad812` test: don't override SOLBITD and SOLBITCLI if they're set (Wladimir J. van der Laan)
...
- #6401 `6db53b4` Add SOLBITD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
...
- #6504 `b6fee6b` Rationalize currency unit to "SBT" (Ross Nicoll)
