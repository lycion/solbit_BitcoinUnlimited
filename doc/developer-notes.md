...
- To auto-format a file:
  ...
  ========================
  Add the following to your $HOME/.gitconfig or .git/config in the Solbit directory:
  ```
[filter "solbit-clang-format"]
...
```
   Do not fix other people's formatting issues in your PR, since it distracts from the content!  Let the SolbitJanitor clean up other people's mess.
...
Run with the -testnet option to run with "play solbits" on the test network, if you
...
Solbit Unlimited is a multithreaded application, and deadlocks or other multithreading bugs
...
Solbit Unlimited can be compiled with the lisbtmalloc allocation library and
memory profiling tool.  First install lisbtmalloc either from source here
...
- SolbitMiner : Generates solbits (if wallet is enabled).
...
However, in open source software such as Solbit Unlimited, where everyone uses
...
pay attention to for reviewers of Solbit Unlimited code.

General Solbit Unlimited
----------------------
...
- For `strprintf`, `LOG`, `LOGA` formatting characters don't need size specifiers
  ...
  - *Rationale*: Solbit Unlimited uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Solbit and Solbit Cash, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Solbit Unlimited so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/solbit-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/solbit-core/ctaes ; actively maintained by Core contributors.
...
file descriptors it will cause problems with Solbit's `select()` loop, because
...
compatibility between nodes. This happened in Solbit 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Solbit's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:solbit/solbit.git
...
Commit [`bb81e173`](https://github.com/solbit/solbit/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `solbit-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `solbit-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Solbit, the wallet was always
