...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-solbit-host/README.md).
...
Clone the git repositories for solbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/Solbit/Solbit
```
...
in the solbit repository under 'Fetch and build inputs' to install sources which require
...
To build Solbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the solbit repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/Solbit/Solbit
...
For example:
```bash
URL=https://github.com/laanwj/solbit.git
...
and the solbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../solbit/contrib/gitian-descriptors/*|sort|uniq )
...
SBTPATH=/some/root/path/solbit.git
SIGPATH=/some/root/path/solbit-detached-sigs.git

./bin/gbuild --url solbit=${SBTPATH},signature=${SIGPATH} ../solbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/solbit-osx-build.assert
```
...
[solbit/gitian.sigs](https://github.com/solbit/gitian.sigs/) repository, or if that's not possible create a pull
