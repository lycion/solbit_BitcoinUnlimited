...
# based on https://github.com/ftrader/solbit-misc-contrib/blob/master/bisection-tools/bisect_probabilistic_with_timeout.sh
# relicensed by freetrader under the MIT software license (see below)
#########################################################################
# Copyright (c) 2017 The Solbit Unlimited developers
...
#
# 1. CAUTION!!!! This kills any running solbitd's as part of cleanup!
...
# optional tool for use with "test_solbit" tests
...
# sequentially.
PARALLEL_TEST_TOOL=gtest-parallel-solbit
...
TEST_COMMAND=${3:-"src/test/test_solbit"}
...
# helper function to check if parameter is some form of test_solbit
# this function has been dumbed down for now.
is_test_solbit()
{
    if [ "$1" = "src/test/test_solbit" ]
    then
        return 0
    else
        return 1
...
do
    ...
    # check if the test is src/test/test_solbit
    is_test_solbit "${TEST_COMMAND}"
    if [ $? -eq 0 ]
    then
