...
###   This script attempts to download the signature file SHA256SUMS.asc from solbit.org
...
WORKINGDIR="/tmp/solbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://solbit.org/bin/"
VERSIONPREFIX="solbit-unlimited-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'solbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Solbit binary release signing key installed?"
   fi
