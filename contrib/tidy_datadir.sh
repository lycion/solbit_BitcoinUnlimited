#!/bin/bash
# Copyright (c) 2013 The Solbit Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Solbit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Solbit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Solbit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Solbit 0.7 datadir."
    ;;
  3)
    echo "Detected Solbit pre-0.8 datadir."
    ;;
  4)
    echo "Detected Solbit 0.8 datadir."
    ;;
esac
