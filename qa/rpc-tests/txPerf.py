...
class TransactionPerformanceTest(SolbitTestFramework):

    def setup_chain(self,solbitConfDict=None, wallets=None):
        logging.info("Initializing test directory "+self.options.tmpdir)
        initialize_chain_clean(self.options.tmpdir, 3, solbitConfDict, wallets)

    def setup_network(self, split=False):
        self.nodes = start_nodes(3, self.options.tmpdir,timewait=60*60)

        #connect to a local machine for debugging
        #url = "http://solbitrpc:DP6DvqZtqXarpeNWyN3LZTFchCCyCUuHwNF7E8pX99x1@%s:%d" % ('127.0.0.1', 19772)
...
def Test():    
    ...
    tpt.main(["--nocleanup","--tmpdir=/ramdisk/test"],solbitConf)
