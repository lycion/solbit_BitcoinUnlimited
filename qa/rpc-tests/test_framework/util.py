...
class NoConfigValue:
    """ Use to remove the specific configure parameter and value when writing to solbit.conf"""
    def __init__(self):
        pass
...
def enable_mocktime():
    # Set the mocktime to be after the Solbit Cash fork so
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting SBT values"""
...
def initialize_datadir(dirname, n, solbitConfDict=None, wallet=None, bins=None):
    ...
    if solbitConfDict: defaults.update(solbitConfDict)
...
def wait_for_solbitd_start(process, url, i):
    '''
    Wait for solbitd to start. This means that RPC is accessible and fully initialized.
    Raise an exception if solbitd exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('solbitd exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir,solbitConfDict=None,wallets=None, bins=None):
    """
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        # Create cache directories, run solbitds:
        for i in range(4):
            for retry in range(4):
                try:
                    datadir=initialize_datadir("cache", i, solbitConfDict, wallets, bins)
                    if bins:
                        args = [ bins[i], "-keypool=1", "-datadir="+datadir ]
                    else:
                        args = [ os.getenv("SOLBITD", "solbitd"), "-keypool=1", "-datadir="+datadir ]
                    if i > 0:
                        args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
                    solbitd_processes[i] = subprocess.Popen(args)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: solbitd started, waiting for RPC to come up")
                    wait_for_solbitd_start(solbitd_processes[i], rpc_url(i), i)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: RPC succesfully started")
                    break
                except Exception as exc:
                    logging.error("Error bringing up solbitd #%d (initialize_chain, directory %s), this might be retried. Problem is:", i, test_dir)
                    do_and_ignore_failure(solbitd_processes[i].kill)
                    traceback.print_exc(file=sys.stdout)
                    remap_ports(i)
            else:
                raise Exception("Couldn't start solbitd even with retries on different ports (initialize_chain).")
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i,solbitConfDict,wallets[i] if wallets else None) # Overwrite port/rpcport in solbit.conf

def initialize_chain_clean(test_dir, num_nodes, solbitConfDict=None, wallets=None):
    """
    Create an empty blockchain and num_nodes wallets.
    Useful if a test case wants complete control over initialization.
    """
    for i in range(num_nodes):
        datadir=initialize_datadir(test_dir, i, solbitConfDict, wallets)
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("SOLBITD", "solbitd")
    ...
    for retry in range(4):
        try:
            solbitd_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: solbitd started, waiting for RPC to come up")
            url = rpc_url(i, rpchost)
            wait_for_solbitd_start(solbitd_processes[i], url, i)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: RPC succesfully started")
            break
        except Exception as exc:
            logging.error("Error bringing up solbitd #%d (start_node, directory %s), this might be retried. Problem is:", i, dirname)
            do_and_ignore_failure(solbitd_processes[i].kill)
            traceback.print_exc(file=sys.stdout)
            remap_ports(i)
            fixup_ports_in_configfile(i)
    else:
        raise Exception("Couldn't start solbitd even with retries on different ports (start_node).")
...
def stop_node(node, i):
    ...
    solbitd_processes[i].wait(timeout=SOLBITD_PROC_WAIT_TIMEOUT)
    del solbitd_processes[i]
...
def wait_solbitds():
    # Wait for all solbitds to cleanly exit
    for solbitd in solbitd_processes.values():
        solbitd.wait(timeout=SOLBITD_PROC_WAIT_TIMEOUT)
    solbitd_processes.clear()
...
def split_transaction(node, prevouts, toAddrs, txfeePer=DEFAULT_TX_FEE_PER_BYTE,**kwargs):
    ...
    try:  # try finally block to put the decimal precision back to what it was prior to this routine
      decimal.getcontext().prec = 8 + 8 # 8 digits to get to 21million, and each solbit is 100 million satoshis
      amount = Decimal(0)
      iamount = 0
      count = 0
      for tx in prevouts:
        inp.append({"txid":str(tx["txid"]),"vout":tx["vout"]})
        amount += tx["amount"]*Decimal(SBT)
        iamount += int(tx["amount"]*Decimal(SBT))
        count += 1
      ...
      while 1:
          ...
          for a in toAddrs[0:-1]:
            if PerfectFractions:
              outp[str(a)] = str(amtPer/Decimal(SBT))
            else:
              outp[str(a)] = float(amtPer/SBT)

          a = toAddrs[-1]
          amtPer = (amount - ((len(toAddrs)-1)*amtPer)) - txfee
          # print "final amt: ", amtPer
          if PerfectFractions:
              outp[str(a)] = str(amtPer/SBT)
          else:
              outp[str(a)] = float(amtPer/SBT)
