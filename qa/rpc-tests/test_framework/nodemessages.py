...
COIN = 100000000  # 1 sbt in satoshis
...
def encodeSolbitAddress(prefix, data):
    data2 = prefix + data
...
def hash256(s):
    """Return the double SHA256 hash (what solbit typically uses) of the passed binary data
...
def deser_string(f):
    """Convert an array of bytes in the solbit P2P protocol format into a string
...
def ser_string(s):
    """convert a string into an array of bytes (in the solbit network format)
...
# Objects that map to solbitd objects, which can be serialized/deserialized
...
class msg_headers(object):
    ...
    def deserialize(self, f):
        # comment in solbitd indicates these should be deserialized as blocks
