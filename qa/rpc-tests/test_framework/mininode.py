...
# Copyright (c) 2010-2016 The Solbit Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

#
# mininode.py - Solbit P2P network half-a-node
#
# This python code was modified from ArtForz' public domain  half-a-node, as
# found in the mini-node branch of http://github.com/jgarzik/pynode.
#
# NodeConn: an object which manages p2p connectivity to a solbit node
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=1, solbitCash=True):
        self.solbitCash = solbitCash
        if self.solbitCash:
            self.MAGIC_BYTES = self.CASH_MAGIC_BYTES
        else:
            self.MAGIC_BYTES = self.SBT_MAGIC_BYTES
        ...
        print('MiniNode: Connecting to Solbit Node IP # ' + dstaddr + ':'
              + str(dstport))
        try:
    ...
    def got_data(self):
        ...
        try:
            while True:
                ...
                if nowLen < 4:
                    return
                if (self.recvbuf[:4] != self.MAGIC_BYTES[self.network]) and (self.recvbuf[:4] != self.SBT_MAGIC_BYTES[self.network]):
