...
def init(libsolbitcashfile=None):
    global cashlib
    if libsolbitcashfile is None:
        libsolbitcashfile = "libsolbitcash.so"
        try:
            cashlib = CDLL(libsolbitcashfile)
        except OSError:
            import os
            dir_path = os.path.dirname(os.path.realpath(__file__))
            cashlib = CDLL(dir_path + os.sep + libsolbitcashfile)
    else:
        cashlib = CDLL(libsolbitcashfile)
    if cashlib is None:
        raise Error("Cannot find %s shared library", libsolbitcashfile)
...
def signTxInput(tx, inputIdx, inputAmount, prevoutScript, key, sigHashType=SIGHASH_FORKID | SIGHASH_ALL):
    ...
    """
    assert (sigHashType & SIGHASH_FORKID) > 0, "Did you forget to indicate the solbit cash hashing algorithm?"
    if type(tx) == str:
        tx = unhexlify(tx)
    elif type(tx) != bytes:
        tx = tx.serialize()
    ...
    if type(inputAmount) is decimal.Decimal:
        inputAmount = int(inputAmount * SBT)
