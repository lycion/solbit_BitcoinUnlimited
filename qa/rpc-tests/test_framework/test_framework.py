...
class SolbitTestFramework(object):
    ...
    def setup_chain(self,solbitConfDict=None, wallets=None):
        """
        Sets up the blockchain for the solbit nodes.  It also sets up the daemon configuration.
        solbitConfDict:  Pass a dictionary of values you want written to solbit.conf.  If you have a key with multiple values, pass a list of the values as the value, for example:
        ...
        """
        print("Initializing test directory ", self.options.tmpdir, "Solbit conf: ", str(solbitConfDict), "walletfiles: ", wallets)
        initialize_chain(self.options.tmpdir,solbitConfDict, wallets, self.bins)
    ...
    def main(self,argsOverride=None,solbitConfDict=None,wallets=None):
        """
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave solbitds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop solbitds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing solbitd/solbit-cli (default: %default)")
        ...
        try:
            ...
            if self.setup_chain.__defaults__ is None:
              ...
              self.setup_chain(solbitConfDict, wallets)
        ...
        if not self.options.noshutdown:
            ...
            print("Note: solbitds were not stopped and may still be running")
...
# Test framework for doing p2p comparison testing, which sets up some solbitd
# binaries:
# 1 binary: test binary
# 2 binaries: 1 test binary, 1 ref binary
# n>2 binaries: 1 test binary, n-1 ref binaries

class ComparisonTestFramework(SolbitTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("SOLBITD", "solbitd"),
                          help="solbitd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("SOLBITD", "solbitd"),
                          help="solbitd binary to use for reference nodes (if any)")

    def setup_chain(self,solbitConfDict=None, wallets=None):  # BU add config params
        print("Initializing test directory ", self.options.tmpdir)
        initialize_chain_clean(self.options.tmpdir, self.num_nodes,solbitConfDict, wallets)
