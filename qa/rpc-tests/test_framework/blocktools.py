...
def solbitAddress2bin(sbtAddress):
    """convert a solbit address to binary data capable of being put in a CScript"""
    # chop the version and checksum out of the bytes of the address
    return decodeBase58(sbtAddress)[1:-4]
...
def createWastefulOutput(sbtAddress):
    """ Warning: Creates outputs that can't be spent by solbitd"""
...
this is junk data. this is junk data. this is junk data. this is junk data. this is junk data."""
    ret = CScript([data, OP_DROP, OP_DUP, OP_HASH160, solbitAddress2bin(sbtAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def p2pkh(sbtAddress):
    """ create a pay-to-public-key-hash script"""
    ret = CScript([OP_DUP, OP_HASH160, solbitAddress2bin(sbtAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def createrawtransaction(inputs, outputs, outScriptGenerator=p2pkh):
    """
    Create a transaction with the exact input and output syntax as the solbit-cli "createrawtransaction" command.
    If you use the default outScriptGenerator, this function will return a hex string that exactly matches the
    output of solbit-cli createrawtransaction.

    But this function is extended beyond solbit-cli in the following ways:
    ...
    for addr, amount in pairs:
        if callable(addr):
            tx.vout.append(CTxOut(amount * SBT, addr()))
        elif type(addr) is list:
            tx.vout.append(CTxOut(amount * SBT, CScript(addr)))
        elif addr == "data":
            tx.vout.append(CTxOut(0, CScript([OP_RETURN, unhexlify(amount)])))
        else:
            tx.vout.append(CTxOut(amount * SBT, outScriptGenerator(addr)))
    tx.rehash()
