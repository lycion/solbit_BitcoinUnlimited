...
invalidOpReturn = hexlify(b'Solbit: A Peer-to-Peer Electronic Cash System')

class BUIP055Test (SolbitTestFramework):
    def __init__(self, extended=False):
        self.extended = extended
        SolbitTestFramework.__init__(self)
    ...
    def testDefaults(self):
        for n in self.nodes:
            ...
            if int(nodeInfo["localservices"],16)&NODE_SOLBIT_CASH:
                assert(t['mining.forkTime'] == 1501590000)  # Solbit Cash release REQ-2
            else:
    ...
    def testCli(self):
        ...
        # if this is a solbit cash build, we need to do the cash defaults on our old chain node
        if int(nodeInfo["localservices"],16)&NODE_SOLBIT_CASH:
    ...
    def testNetMagic(self):
        ...
        buCashNode = BasicSBCashNode()
        buCashNode.connect(0,'127.0.0.1', p2p_port(0), self.nodes[0])
        if int(info["localservices"],16)&NODE_SOLBIT_CASH:
            try: # Accept BU cash nodes if running SBT node
                buCashNode.cnxns[0].wait_for_buverack()
            except DisconnectedError:
                assert(not "should not have disconnected a solbit cash node")
        else:
            try: # do not accept BU cash nodes if running SBT node
                buCashNode.cnxns[0].wait_for_buverack()
                assert(not "should have disconnected a solbit cash node")
            except DisconnectedError:
...
def Test():
    ...
    t.main(["--tmpdir=/ramdisk/test","--nocleanup","--noshutdown"], solbitConf, None)  # , "--tracerpc"])
