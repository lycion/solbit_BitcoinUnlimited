...
`qa/pull-tester/test_framework/test_framework.py:SolbitTestFramework.main`.
...
#Set env vars
if "SOLBITD" not in os.environ:
    os.environ["SOLBITD"] = BUILDDIR + '/src/solbitd' + EXEEXT
if "SOLBITCLI" not in os.environ:
    os.environ["SOLBITCLI"] = BUILDDIR + '/src/solbit-cli' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_SOLBITD == 1):
    print("No rpc tests to run. Wallet, utils, and solbitd must all be enabled")
    sys.exit(0)
...
def runtests():
    ...
    if(ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_SOLBITD == 1):
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie solbitds, we can apply a
        ...
        while True:
            ...
            for j in self.jobs:
                ...
                if proc.poll() is not None:
                    ...
                    stderr_filtered = re.sub(r"Error: Unable to bind to 0.0.0.0:[0-9]+ on this computer\. Solbit Unlimited Cash Edition is probably already running\.",
                                             "", stderr_filtered)
...
class RPCCoverage(object):
    ...
    commands per `solbit-cli help` (`rpc_interface.txt`).
