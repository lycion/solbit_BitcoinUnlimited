...
#endif
    GenerateSolbits(false, 0, Params());
    StopNode();
    StopTorControl();
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-minrelaytxfee"))
    {
        InitWarning(_("Config option -minrelaytxfee is no longer supported.  To set the limit "
                      ...
                      "To convert -minrelaytxfee, which is specified  in SBT/KB, to -minlimtertxfee, "
    ...
    // UAHF - SolbitCash service bit
    nLocalServices |= NODE_SOLBIT_CASH;
    ...
    LOGA("Using config file %s\n", GetConfigFile(GetArg("-conf", SOLBIT_CONF_FILENAME)).string());
...
// ********************************************************* Step 7: load wallet
    ...
    // It is possible that we need to override the old settings in QT and solbit.conf.
