...
void UnlimitedSetup(void)
{
    ...
    GenerateSolbits(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), Params());
}
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Solbit Unlimited Developers"), COPYRIGHT_YEAR)) +
           "\n\n" +
           FormatParagraph(strprintf(_("Portions Copyright (C) 2009-%i The Solbit Core Developers"), COPYRIGHT_YEAR)) +
           "\n\n" +
           FormatParagraph(strprintf(_("Portions Copyright (C) 2014-%i The Solbit XT Developers"), COPYRIGHT_YEAR)) +
...
void static SolbitMiner(const CChainParams &chainparams)
{
    LOGA("SolbitMiner started\n");
    ...
    try
    {
        ...
        while (true)
        {
            ...
            {
                LOGA("Error in SolbitMiner: Keypool ran out, please call keypoolrefill before restarting the "
                     "mining thread\n");
                return;
            }
            ...
            LOGA("Running SolbitMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
                pblock->GetBlockSize());
    ...
    {
        LOGA("SolbitMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LOGA("SolbitMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateSolbits(bool fGenerate, int nThreads, const CChainParams &chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SolbitMiner, boost::cref(chainparams)));
}
...
UniValue getgenerate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getgenerate\n"
                            ...
                            std::string(SOLBIT_CONF_FILENAME) +
                            " setting gen)\n"
...
UniValue setgenerate(const UniValue &params, bool fHelp)
{
    ...
    GenerateSolbits(fGenerate, nGenProcLimit, Params());
...
extern UniValue getminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getminercomment\n"
                            ...
                            "after the Solbit Unlimited parameters."
                            "\nResult\n"
...
extern UniValue setminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("setminercomment\n"
                            ...
                            "the Solbit Unlimited parameters.\n Comments that are too long will be truncated."
                            "\nExamples:\n" +
                            HelpExampleCli("setminercomment", "\"solbit is fundamentally emergent consensus\"") +
                            HelpExampleRpc("setminercomment", "\"solbit is fundamentally emergent consensus\""));
...
UniValue validateblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.solbit.it/wiki/BIP_0022 for full specification.\n"
...
UniValue getaddressforms(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error("getaddressforms \"address\"\n"
                            ...
                            "\"solbitcash\": \"solbitcash prefixed address\",\n"
    ...
    node.push_back(Pair("solbitcash", cashAddr));
