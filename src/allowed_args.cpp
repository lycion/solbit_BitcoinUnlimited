...
// Copyright (c) 2017-2018 The Solbit Unlimited developers
...
// These globals are needed here so solbit-cli can link
const std::string CURRENCY_UNIT = "SBT";
const std::string DEFAULT_TOR_CONTROL = "127.0.0.1:9051";
const char DEFAULT_RPCCONNECT[] = "127.0.0.1";

// Variables for traffic shaping.  Needed here so solbit-cli can link
...
static void addConfigurationLocationOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("Configuration location options:"))
        .addArg(
            "conf=<file>", requiredStr, strprintf(_("Specify configuration file (default: %s)"), SOLBIT_CONF_FILENAME))
        .addArg(
...
#ifndef WIN32
        .addArg("pid=<file>", requiredStr, strprintf(_("Specify pid file (default: %s)"), SOLBIT_PID_FILENAME))
#endif
...
static void addWalletOptions(AllowedArgs &allowedArgs)
{
    ...
    allowedArgs.addHeader(_("Wallet options:"))
        ...
        .addArg("usecashaddr", optionalBool,
            _("Use Solbit Cash Address for destination encoding (Activates by default Jan 14, 2017)"));
#endif
}
...
static void addRpcServerOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("RPC server options:"))
        ...
        // Although a node does not use rpcconnect it must be allowed because SolbitCli also uses the same config file
...
// solbit-cli does not know about tweaks so we have to silently ignore unknown options
SolbitCli::SolbitCli() : AllowedArgs(true)
{
    addHelpOptions(*this);
...
Solbitd::Solbitd(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_SOLBITD, pTweaks); }
SolbitQt::SolbitQt(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_SOLBIT_QT, pTweaks); }
SolbitTx::SolbitTx() : AllowedArgs(false)
{
    addHelpOptions(*this);
    addChainSelectionOptions(*this);
    ...
    addHeader(_("Transaction options:"))
        ...
        .addDebugArg("", optionalBool, "Read hex-encoded solbit transaction from stdin.");
}

ConfigFile::ConfigFile(CTweakMap *pTweaks) : AllowedArgs(false)
{
    // Merges all allowed args from SolbitCli, Solbitd, and SolbitQt.
    // Excludes args from SolbitTx, because solbit-tx does not read
    // from the config file. Does not set a help message, because the
    // program does not output a config file help message anywhere.

    SolbitCli solbitCli;
    Solbitd solbitd(pTweaks);
    SolbitQt solbitQt;

    m_args.insert(solbitCli.getArgs().begin(), solbitCli.getArgs().end());
    m_args.insert(solbitd.getArgs().begin(), solbitd.getArgs().end());
    m_args.insert(solbitQt.getArgs().begin(), solbitQt.getArgs().end());
}
