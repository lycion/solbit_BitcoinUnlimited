...
UniValue generatetoaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error("generatetoaddress numblocks address (maxtries)\n"
                            ...
                            "\nArguments:\n"
                            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
                            "2. address    (string, required) The address to send the newly generated solbit to.\n"
...
// NOTE: Unlike wallet RPC (which use SBT values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/solbit/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/solbit/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
...
UniValue estimatesmartfee(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("estimatesmartfee nblocks\n"
                            ...
                            "\nResult:\n"
                            "{\n"
                            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in SBT)\n"
