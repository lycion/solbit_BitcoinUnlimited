...
// BU add lockstack stuff here for solbit-miner, because I need to carefully
...
class SolbitMinerArgs : public AllowedArgs::SolbitCli
{
public:
    SolbitMinerArgs(CTweakMap *pTweaks = nullptr)
    {
...
static UniValue CpuMineBlock(unsigned int searchDuration, const UniValue &params, bool &found)
{
    ...
    while ((GetTime() < start + searchDuration) && !found)
    {
        ...
        // and the block will be rejected.  So do not advance time (let it be advanced by solbitd every time we
        // request a new block).
    ...
    while (0 != nblocks)
    {
        ...
        if (strPrint != "")
        {
            ...
            else
            {
                ...
                // The result is sent to solbitd above when the loop gets to it.
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        std::string appname("solbit-miner");
        std::string usage = "\n" + _("Usage:") + "\n" + "  " + appname + " [options] " + "\n";
        int ret = AppInitRPC(usage, SolbitMinerArgs(), argc, argv);
