...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = GetTempPath() / strprintf("test_solbit_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(1 << 30)));
...
struct StartupShutdown
{
    StartupShutdown()
    {
        ...
        optDef.add_options()("testhelp", "program options information")(
            ...
            "log_solbit", value<std::string>()->required(), "solbit logging destination (console, none)");
        variables_map opts;
        ...
        if (opts.count("log_solbit"))
        {
            std::string s = opts["log_solbit"].as<std::string>();
            if (s == "console")
            {
                /* To enable this, add
                   -- --log_solbit console
                   to the end of the test_solbit argument list. */
