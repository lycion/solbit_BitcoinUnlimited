// Copyright (c) 2012-2015 The Solbit Core developers
...
static void ResetArgs(const std::string &strArg, Kind kind = SOLBITD)
{
    ...
    vecArg.insert(vecArg.begin(), "testsolbit");
    ...
    else if (kind == SOLBITD)
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::Solbitd(&tweaks));
    else
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::SolbitCli());
}
...
BOOST_AUTO_TEST_CASE(tweakArgs)
{
    ...
    // Test solbit-cli accepts unknown tweaks
    ResetArgs("-some.tweak=something", SOLBIT_CLI);
