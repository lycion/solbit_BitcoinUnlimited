...
#include "solbitaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Solbit address (e.g. %1)")
                                   .arg(QString::fromStdString(DummyAddress(params, GetConfig()))));
    widget->setValidator(new SolbitAddressEntryValidator(params.CashAddrPrefix(), parent));
    widget->setCheckValidator(new SolbitAddressCheckValidator(parent));
}
...
QString solbitURIScheme(const CChainParams &params, bool useCashAddr)
{
    if (!useCashAddr)
    {
        return "solbitcash";
...
QString solbitURIScheme(const Config &cfg)
{
    return solbitURIScheme(cfg.GetChainParams(), cfg.UseCashAddrEncoding());
}
...
bool parseSolbitURI(const QString &scheme, const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if (!i->second.isEmpty())
            {
                if (!SolbitUnits::parse(SolbitUnits::SBT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseSolbitURI(const QString &scheme, QString uri, SendCoinsRecipient *out)
{
    //
    //    Cannot handle this later, because solbitcash://
    ...
    return parseSolbitURI(scheme, uriInstance, out);
}

QString formatSolbitURI(const Config &cfg, const SendCoinsRecipient &info)
{
    QString ret = info.address;
    if (!cfg.UseCashAddrEncoding())
    {
        // prefix address with uri scheme for base58 encoded addresses.
        ret = (solbitURIScheme(cfg) + ":%1").arg(ret);
    }
    int paramCount = 0;
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1")
                   .arg(SolbitUnits::format(SolbitUnits::SBT, info.amount, false, SolbitUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Solbit (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Solbit (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("solbit-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=Solbit (%s)\n", chain);
...
bool GetStartOnSystemStartup()
{
    CFURLRef solbitAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, solbitAppUrl);
    // findStartupItemInList retains the item it returned, need to release
    if (foundItem)
        CFRelease(foundItem);
    CFRelease(loginItems);
    CFRelease(solbitAppUrl);
    return !!foundItem;
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem)
    {
        ...
        LSSharedFileListInsertItemURL(
            loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, solbitAppUrl, NULL, NULL);
    }
...
QString uriPrefix() { return "solbitcash"; }
} // namespace GUIUtil
