...
const int SOLBIT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
// BIP70 payment protocol messages
const char *BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char *BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char *BIP71_MIMETYPE_PAYMENT = "application/solbitcash-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/solbitcash-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST = "application/solbitcash-paymentrequest";
...
{
    const QString scheme = GUIUtil::solbitURIScheme(params, useCashAddr);
    ...
    if (!GUIUtil::parseSolbitURI(scheme, arg, &r))
    {
        return {};
    }
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString &r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SOLBIT_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SOLBIT_IPC_CONNECT_TIMEOUT);
    ...
    : QObject(parent), saveURIs(true), uriServer(0), netManager(0), optionsModel(0)
{
    ...
    // on Mac: sent when you click solbitcash: links
...
//
// OSX-specific way of handling solbitcash: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in solbitcash: URIs
...
bool PaymentServer::handleURI(const QString &scheme, const QString &s)
{
    ...
    if (GUIUtil::parseSolbitURI(scheme, s, &recipient))
    {
    ...
    else
    {
        Q_EMIT message(tr("URI handling"), tr("URI cannot be parsed! This can be caused by an invalid "
                                              "Solbit address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
    }
...
void PaymentServer::handleURIOrFile(const QString &s)
{
    ...
    // solbitcash: CashAddr URI
    QString schemeCash = GUIUtil::solbitURIScheme(Params(), true);
    if (handleURI(schemeCash, s))
    {
        return;
    }

    // solbitcash: Legacy URI
    QString schemeLegacy = GUIUtil::solbitURIScheme(Params(), false);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request, SendCoinsRecipient &recipient)
{
    ...
    Q_FOREACH (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos)
    {
        ...
        {
            // Unauthenticated payment requests to custom solbit addresses are
        ...
        // Solbit amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust())
        {
            Q_EMIT message(tr("Payment request error"),
                tr("Requested payment amount of %1 is too small (considered dust).")
                    .arg(SolbitUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
