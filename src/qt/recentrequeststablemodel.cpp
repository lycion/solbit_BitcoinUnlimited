    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return SolbitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount,
                    false, SolbitUnits::separatorNever);
            else
                return SolbitUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
QString RecentRequestsTableModel::getAmountTitle()
{
    ...
    {
        amountTitle += " (" + SolbitUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")";
    }
    return amountTitle;
}
