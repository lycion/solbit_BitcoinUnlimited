...
{
    return QCoreApplication::translate("solbit-unlimited", psz).toStdString();
}
    ...
    QTranslator &translator)
{
    ...
    // Load e.g. solbit_de.qm (shortcut "de" needs to be defined in solbit.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. solbit_de_DE.qm (shortcut "de_DE" needs to be defined in solbit.qrc)
...
/** Class encapsulating Solbit startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class SolbitCore : public QObject
{
    Q_OBJECT
public:
    explicit SolbitCore();
...
class SolbitApplication : public QApplication
{
    Q_OBJECT
public:
    explicit SolbitApplication(int &argc, char **argv);
    ~SolbitApplication();
    ...
    /// Get window identifier of QMainWindow (SolbitGUI)
...
SolbitCore::SolbitCore() : QObject() {}
void SolbitCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(strMiscWarning));
}

void SolbitCore::initialize(Config *cfg)
{
    Config &config(*cfg);
    try
    {
...
SolbitApplication::SolbitApplication(int &argc, char **argv)
...
SolbitApplication::~SolbitApplication()
{
    if (coreThread)
    {
...
#ifdef ENABLE_WALLET
void SolbitApplication::createPaymentServer() { paymentServer = new PaymentServer(this); }
#endif

void SolbitApplication::createPlatformStyle()
{
...
void SolbitApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(NULL, resetSettings);
    unlimitedModel = new UnlimitedModel(); // BU
}

void SolbitApplication::createWindow(const Config *config, const NetworkStyle *networkStyle)
{
    window = new SolbitGUI(config, platformStyle, networkStyle, 0);
...
void SolbitApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void SolbitApplication::startThread()
{
    if (coreThread)
        return;
    coreThread = new QThread(this);
    SolbitCore *executor = new SolbitCore();
...
void SolbitApplication::parameterSetup()
{
    InitLogging();
    InitParameterInteraction();
}

void SolbitApplication::requestInitialize(Config &config)
{
...
void SolbitApplication::requestShutdown()
{
...
void SolbitApplication::initializeResult(int retval)
{
    ...
    if (retval)
    {
        ...
        if (pwalletMain)
        {
            ...
            window->addWallet(SolbitGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SolbitGUI::DEFAULT_WALLET);
        ...
        // solbitcash: URIs or payment requests:
...
void SolbitApplication::shutdownResult(int retval)
{
    qDebug() << __func__ << ": Shutdown result: " << retval;
    quit(); // Exit main loop after shutdown finished
}

void SolbitApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception",
        SolbitGUI::tr("A fatal error occurred. Solbit can no longer continue safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId SolbitApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    SolbitApplication app(argc, argv);
    ...
    AllowedArgs::SolbitQt allowedArgs(&tweaks);
    try
    {
    ...
    catch (const std::exception &e)
    {
        QMessageBox::critical(
            0, QObject::tr("Solbit"), QObject::tr("Error: Cannot parse program options: %1.").arg(e.what()));
        return EXIT_FAILURE;
    }
    ...
    // For SBCash, first try to migrate from SBT BU settings
    ...
    // This is necessary in case the user wants to run side-by-side SBT chain and SBT chain nodes
    ...
    // Use a different app name for SBCash to enable side-by-side installations which won't
    ...
    /// 7. Determine availability of data directory and parse solbit.conf
    ...
    // solbitcash: links repeatedly have their payment requests routed to this
    // process:
