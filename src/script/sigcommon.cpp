    ...
    size_t *nHashedOut)
{
    if (nIn >= txTo.vin.size())
    {
        ...
        // returned here is, however, due to further omissions in CheckSig, part of the pre-SBT
        // consensus rule set and needs to be left as-is.
        // See also: https://lists.linuxfoundation.org/pipermail/solbit-dev/2014-November/006878.html
...
static uint256 SignatureHashSolbitCash(const CScript &scriptCode,
    const CTransaction &txTo,
    unsigned int nIn,
    uint32_t nHashType,
    const CAmount &amount,
    ...
    size_t *nHashedOut)
{
    ...
    {
        return SignatureHashSolbitCash(scriptCode, txTo, nIn, nHashType, amount, nHashedOut);
    }
