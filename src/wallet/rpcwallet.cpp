...
UniValue getaccountaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Solbit address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue getaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("getaccount \"solbitaddress\"\n"
...
UniValue sendtoaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"solbitaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            ...
            "being sent.\n"
            "                             The recipient will receive less solbits than you enter in the amount "
            "field.\n"
            "\nResult:\n"
...
UniValue getreceivedbyaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error("getreceivedbyaddress \"solbitaddress\" ( minconf )\n"
                            "\nReturns the total amount received by the given solbitaddress in transactions with at "
...
UniValue sendfrom(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a solbit address." +
            HelpRequiringPassphrase() +
            "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The solbit address is the key, the numeric amount (can be "
            "string) in " +
            CURRENCY_UNIT +
            " is the value\n"
            "      ,...\n"
            ...
            "                           Those recipients will receive less solbits than you enter in their "
    ...
    for (const std::string &name_ : keys)
    {
        ...
        if (!IsValidDestination(dest))
        {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Solbit address: ") + name_);
        }
...
UniValue encryptwallet(const UniValue &params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending solbit\n" +
    ...
    StartShutdown();
    return "wallet encrypted; Solbit server stopping, restart to run with encrypted wallet. The keypool has been "
