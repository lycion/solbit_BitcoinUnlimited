...
const char *const SOLBIT_CONF_FILENAME = "solbit.conf";
const char *const SOLBIT_PID_FILENAME = "solbitd.pid";
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Solbit";
#else
    // Unix
...
{
    fs::ifstream streamConfig(GetConfigFile(GetArg("-conf", SOLBIT_CONF_FILENAME)));
    if (!streamConfig.good())
        return; // No solbit.conf file is OK
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", SOLBIT_PID_FILENAME));
